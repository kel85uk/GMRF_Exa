...
double* fieldData_Residual_GMRF_8_p1 = (&fieldData_Residual_GMRF[8][(y*260)]);
int x = iterationOffsetBegin[0][0];
for (; (x<((iterationOffsetBegin[0][0]+1)&(~1))); x += 1) {
res += (fieldData_Residual_GMRF_8_p1[(x+262)]*fieldData_Residual_GMRF_8_p1[(x+262)]);
}
__m128d vec0 = _mm_set1_pd(0.000000e+00);
for (; (x<(iterationOffsetEnd[0][0]+254)); x += 4) {
/* res += (fieldData_Residual_GMRF_8_p1[(x+262)]*fieldData_Residual_GMRF_8_p1[(x+262)]); */
__m128d vec1 = _mm_loadu_pd((&fieldData_Residual_GMRF_8_p1[(x+262)]));
__m128d vec1_2 = _mm_loadu_pd((&fieldData_Residual_GMRF_8_p1[(x+264)]));
